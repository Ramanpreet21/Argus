import pytest
from argus.core.scorer import RiskScorer
...
def test_scorer_math_accumulation():
    ...
    score = RiskScorer.calculate_score(flags)
    assert score == 65
    assert RiskScorer.determine_risk_level(score) == "MEDIUM"
...
def test_scorer_math_capping():
    ...
    score = RiskScorer.calculate_score(flags)
    assert score == 100
    assert RiskScorer.determine_risk_level(score) == "CRITICAL"
...
def test_scorer_secure_level():
    ...
    score = RiskScorer.calculate_score(flags)
    assert score == 0
    assert RiskScorer.determine_risk_level(score) == "SECURE"

def test_report_generation():
    flags = [
        AnomalyFlag(severity=Severity.HIGH, category=AttackCategory.XSS, description="")
    ]
    report, risk_level = RiskScorer.generate_report(
